...
## 验证
- 建议：脚本就绪时使用 `run_skill_script` 执行 `scripts/audit_coverage.py`、`scripts/verify_entities.py`，否则用 `python_run` 实现等价逻辑。
- 必检：高危问题是否全部绑定修复动作
...
工具调用序列:
1. `python_run` 或 `run_skill_script`（若脚本存在）执行 `scripts/audit_coverage.py`
2. `python_run` 或 `run_skill_script`（若脚本存在）执行 `scripts/verify_entities.py`
