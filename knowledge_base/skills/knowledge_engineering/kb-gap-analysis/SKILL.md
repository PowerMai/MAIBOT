...
| 脚本 | 用途 | 示例 |
|------|------|------|
| detect_gaps.py | 生成 gap_report.json 与可调度任务建议；可传 --input 实体文件或目录、--output 输出路径 | `run_skill_script("kb-gap-analysis", "detect_gaps.py", args=["--output", "knowledge_base/learned/audits/gap_report.json"])` |

## 验证
- 建议：脚本就绪时使用 `run_skill_script` 执行 `scripts/detect_gaps.py`，否则用 `python_run` 实现等价逻辑。
...
工具调用序列:
1. `python_run` 或 `run_skill_script`（若 scripts/detect_gaps.py 存在）生成 `gap_report.json`
