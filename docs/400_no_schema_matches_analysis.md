...
- 报错 **不是** 本后端 FastAPI 返回的，而是 **上游 LLM 服务**（如云端 Qwen 网关 `http://39.170.37.25:63000/v1`）在收到我们的 chat completions 请求后，对 **请求体做 JSON schema 校验** 失败返回的。
...
5. **确认模型与 URL**  
   - 确认当前使用的 `model_id` 和实际请求的 `base_url`（例如是否仍为 `http://39.170.37.25:63000/v1`），排除配错模型或网关导致 schema 不一致。
